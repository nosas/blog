from data_visualization import COLORS_STANDARD
import numpy as np
import keras
...
def _get_multiplier_for_color_randomness():
    ...
    """
    num_colors = len(COLORS_STANDARD)
    ...
    idxs = [i for _, i in sorted(zip(abs_distance, range(num_candidates)))]
    return prime_candidates[idxs[0]]
        ...
        if scores is None or scores[i] > min_score_thresh:
            ...
            if scores is None:
                ...
                if agnostic_mode:
                    ...
                    prime_multiplier = _get_multiplier_for_color_randomness()
                    box_to_color_map[box] = COLORS_STANDARD[
                        (prime_multiplier * track_ids[i]) % len(COLORS_STANDARD)
                    ]
                else:
                    box_to_color_map[box] = COLORS_STANDARD[
                        classes[i] % len(COLORS_STANDARD)
                    ]
