...
train_images = np.asarray(
    [get_img_array_from_filepath(fp) for fp in train_fps], dtype=np.float32
)
...
val_images = np.asarray(
    [get_img_array_from_filepath(fp) for fp in val_fps], dtype=np.float32
)
...
# %% Compile all models
optimizers = [
    tf.keras.optimizers.SGD(learning_rate=LR * 10),
...
# %% Predict an image
fp = DATA_DIR + "\\test\\cog\\cog_sb_gladhander_16.png"
img_array = get_img_array_from_filepath(fp, target_size=(600, 200, 3))
pred = model.predict(img_array.expand_dims(img_array, axis=0))
...
test_images = np.asarray(
    [get_img_array_from_filepath(fp) for fp in test_fps], dtype=np.float32
)
